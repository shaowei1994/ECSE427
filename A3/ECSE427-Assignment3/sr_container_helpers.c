...
//    pid_t chroot = pivot_root(new_root, put_old);
    pid_t chroot = syscall(SYS_pivot_root, new_root, put_old);
    if (chroot < 0) {
        perror("Switch Child Root: ");
        exit(1);
    }
    return 0;
}
...
int setup_syscall_filters()
{
    ...
    if (!seccomp_ctx) {
        fprintf(stderr, "seccomp initiation failed %m \n");
        return EXIT_FAILURE;
    }
    
    // move_pages
    int filter_set_status = seccomp_rule_add(seccomp_ctx, SCMP_FAIL, SCMP_SYS(move_pages), 0);
